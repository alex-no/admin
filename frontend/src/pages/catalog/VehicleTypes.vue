<template>
  <VehicleTypeModal :justCreatedIds="justCreatedIds" />
  <BaseLayout>
...
async function deleteRow(row) {
  if (!confirm(`Видалити «${row.name_uk || row.slug}»?`)) return
  try {
...
function openCreateModal() {
  window.dispatchEvent(new CustomEvent('open-vehicle-type-create'))
}

function openModal(row) {
  window.dispatchEvent(new CustomEvent('open-vehicle-type-edit', { detail: { row } }))
}
...
onMounted(() => {
  ...
  window.addEventListener('vehicle-type-created', (e) => {
    items.value.unshift(e.detail)
    justCreatedIds.value = new Set([...justCreatedIds.value, e.detail.id])
    total.value++
  })

  window.addEventListener('vehicle-type-updated', (e) => {
