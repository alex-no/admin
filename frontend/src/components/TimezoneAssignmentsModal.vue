...
<template>
  ...
  >
    <template #title>
      <div>
        <h6 class="mb-0">{{ t('timezones.assignmentsTitle') }}</h6>
    ...
    <template v-else>
      <div v-if="!asgnList.length" class="text-muted small mb-3">{{ t('timezones.noAssignments') }}</div>
          ...
          <tr v-for="a in asgnList" :key="a.id">
            ...
            <td>
              <button
                ...
                :title="t('timezones.deleteAssignmentTooltip')"
      ...
      <div v-if="canEdit" class="border-top pt-3">
        <div class="fw-semibold small mb-2">{{ t('timezones.addAssignmentTitle') }}</div>
        <div class="row g-2 align-items-end">

          <!-- Type -->
          <div class="col-sm-4">
            <label class="form-label small mb-1">{{ t('filter.type') }}</label>
            <select v-model="addType" class="form-select form-select-sm" @change="onAddTypeChange">
              <option value="">{{ t('timezones.selectPlaceholder') }}</option>
              <option value="country">{{ t('filter.country') }}</option>
              <option value="area_region">{{ t('timezones.areaDistrictOption') }}</option>
              <option value="city">{{ t('timezones.cityLabel') }}</option>
            </select>
          </div>

          <!-- Object selector -->
          <div class="col-sm-6">
            <label class="form-label small mb-1">{{ t('timezones.objectLabel') }}</label>

            <!-- Country: simple select -->
            <select v-if="addType === 'country'"
                    v-model="addObjectId"
                    class="form-select form-select-sm">
              <option :value="null">{{ t('timezones.selectCountryPlaceholder') }}</option>
            ...
            <select v-else-if="addType === 'area_region'"
                    v-model="addObjectId"
                    class="form-select form-select-sm">
              <option :value="null">{{ t('timezones.selectAreaDistrictPlaceholder') }}</option>
              <optgroup v-if="areasList.length" :label="t('timezones.regionsGroupLabel')">
                <option v-for="a in areasList" :key="a.id" :value="a.id">{{ a.name_uk }}</option>
              </optgroup>
              <optgroup v-if="districtsList.length" :label="t('areaRegionList.districtsTitle')">
            ...
            <div v-else-if="addType === 'city'" class="position-relative">
              <input
                ...
                :placeholder="addObjectId ? citySelectedName : t('stoImport.citySearchPlaceholder')"
            ...
            <div v-else class="form-control form-control-sm bg-light text-muted">
              {{ t('timezones.selectTypeFirstHint') }}
            </div>
          </div>
...
function typeLabel(type) {
  return { country: t('filter.country'), area_region: t('cities.areaLabel'), city: t('timezones.cityLabel') }[type] ?? type
}
...
function deleteAssignment(asgn) {
  ...
  if (index === -1) return
  deleteWithUndo({
    message: t('timezones.assignmentDeletedMessage', { name: asgn.name }),
