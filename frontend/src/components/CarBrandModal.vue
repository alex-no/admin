...
<template>
  ...
  >
    <template #title>
      <h6 class="mb-0">{{ modalMode === 'create' ? t('carBrands.newTitle') : t('carBrands.editTitle') }}</h6>
    </template>
    ...
    <div class="mb-3">
      ...
      <select v-if="canEdit" v-model="modalForm.country_id" class="form-select form-select-sm">
        <option :value="null" disabled>{{ t('catalog.selectCountryPlaceholder') }}</option>
    ...
    <div class="mb-3">
      <label class="form-label small mb-1">{{ t('table.name') }} <span class="text-danger">*</span></label>
    ...
    <div class="row g-3 mb-3">
      <div class="col-sm-4">
        ...
        <input
          ...
          :placeholder="t('catalog.foundedYearPlaceholder')"
        />
...
async function save() {
  ...
  try {
    ...
    if (!modalForm.value.name?.trim()) {
      saveError.value = t('carBrands.nameRequiredError')
      return
    }
