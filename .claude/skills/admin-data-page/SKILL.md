...
This codebase used to have two generations of list page; as of 2026-08-07 only one remains:
- **Legacy, hand-rolled** — used to live at `pages/catalog/CarBrands.vue`, `pages/geography/Countries.vue`, and ~15 similar catalog/geography pages, each duplicating its own fetch/sort/paginate logic and opening edit modals via global `CustomEvent`s. This predated `list-framework` and was deleted wholesale (~46 files, commit "Remove some extra pages") once it stopped adding anything the framework demo needed — there is no live example of it left to accidentally copy from.
- **Current, config-driven** (`pages/StoRegistry.vue` using `list-framework/DataListPage.vue`) — the only pattern left in this repo, and the one to build new pages with. If a user request or a stale doc/example names an old-style page, it no longer exists here — treat that as a sign the reference is outdated, not as something to go recreate.
...
- **`{entity}.columns.json`** — one entry per column: `key`, `label`, `type` (resolved via `list-framework/cellTypes.js`: `text` | `select` | `boolean` | `number` | `datetime` | `badge` | `link` | `image` | `phone-list`, or a custom type registered with `registerCellType`), plus `sortable`, `editable`, `align`, `width`, and type-specific options (`options` for `select`, `min`/`max`/`step` for `number`, `emptyLabel` to replace the default «—» placeholder when blank itself carries meaning). `editable: true` + `apiUpdate` gets you inline cell editing for free — no modal required for that field.
...
2. **Is it read-mostly, rich, computed/log-like data** (a stack trace, a feedback message, an analytics breakdown) rather than a form the user edits? Use the **content-component + thin-modal-wrapper split**: a dumb `Xxx­Detail.vue` that just renders the data, plus a `XxxDetailModal.vue` that puts it inside `BaseModal`. Reference: `ErrorLogDetail.vue` + `ErrorLogDetailModal.vue`. (The `FeedbackDetail.vue`/`FeedbackDetailModal.vue` pair that used to be the second example was removed along with the Feedback page on 2026-08-07 — `ErrorLogDetail`/`ErrorLogDetailModal` is now the only live reference for this split.) This split keeps the rendering reusable/testable separately from the modal chrome.
3. **Does one item have few enough fields to fit one simple form**, with create and edit sharing that form? Use a **single-purpose `BaseModal`** with a `modalMode: 'create' | 'edit'` — see the inline modal in `RoleManagement.vue` (`modalMode` ref, `roles.createTitle`/`roles.editTitle`). The standalone `CarBrandModal.vue` this used to point to (opened via a global `CustomEvent` from a legacy-style list page) was removed on 2026-08-07; the current example keeps the modal markup inside the page file rather than a separate component and is triggered with plain local state — prefer that (props/local state, not `CustomEvent`) for any new simple form modal.
...
5. **Is the detail itself a workflow or too large for any modal** (a review/approval queue, a multi-step process, something that wants its own breadcrumb or sub-navigation)? Use a **dedicated route/page** instead of a modal — the exception, not the default. This repo no longer has a live example: `pages/geography/CityTmpReview.vue` and `pages/StoApplications.vue`, which used to demonstrate this shape, were removed on 2026-08-07. Fall back to the general mechanism instead — a normal routed page (`router/index.js` entry + a `menu.json` item; `Breadcrumbs.vue` picks it up automatically, no extra wiring). Only reach for this when a docked-modal genuinely can't hold the content or the workflow needs its own URL structure beyond `?detail=<id>`.
